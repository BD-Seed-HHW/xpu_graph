...
from ..utils import FlashAttentionKwargs, _flash_attention_forward, get_flash_attn_kwargs, logging
...
class P6DenseAttention(nn.Module):
    ...
    def __init__(self, config: P6DenseConfig, layer_idx: Optional[int] = None):
        ...
        if self.config.use_qk_rmsnorm:
            self.q_norm = P6DenseRMSNorm(self.head_dim, eps=config.rms_norm_eps)
            self.k_norm = P6DenseRMSNorm(self.head_dim, eps=config.rms_norm_eps)
...
class P6DenseSdpaAttention(P6DenseAttention):
    ...
    `P6DenseAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "P6DenseModel is using P6DenseSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class P6DenseDecoderLayer(nn.Module):
    def __init__(self, config: P6DenseConfig, layer_idx: int):
        ...
        if config.layer_norm_eps is not None:
            ...
            self.input_layernorm = P6DenseRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.post_attention_layernorm = P6DenseRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class P6DenseModel(P6DensePreTrainedModel):
    ...
    def __init__(self, config: P6DenseConfig):
        ...
        if config.layer_norm_eps is not None:
            ...
            self.norm = P6DenseRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        flash_attn_kwargs: FlashAttentionKwargs = None,
...
class P6DenseForCausalLM(P6DensePreTrainedModel, GenerationMixin):
